...
describe('lib controller', () => {
  ...
  it('returns 400 on findOneAndUpdate', async () => {
  ...
  it('returns 400 on findById with bad id', async () => {
    c = new Controller(model);
    r = await c.findById(req, res) as any;
    expect(r.message).toBe('Find id is invalid');
  });
  it('returns 500 on findById', async () => {
    req.params.id = goodId;
    c = new Controller(model);
    r = await c.findById(req, res) as any;
    expect(r.message).toBe('bad');
  });
  it('returns 400 on findById when no doc is found', async () => {
    req.params.id = goodId;
  ...
  it('does not return the password from findById', async () => {
    req.params.id = goodId;
  ...
  it('returns 500 on create', async () => {
    c = new Controller(model);
    r = await c.create(req, res) as any;
    expect(r.message).toBe('bad');
  });
  it('returns 400 on findByIdAndUpdate with bad id', async () => {
    req.params.id = 'bad';
    c = new Controller(model);
    r = await c.findByIdAndUpdate(req, res) as any;
    expect(r.message).toBe('Update id is invalid');
  });
  it('returns 400 on findByIdAndUpdate with bad userType', async () => {
    req.params.id = goodId;
  ...
  it('returns 500 from findByIdAndUpdate', async () => {
    req.params.id = goodId;
    req.body = { };
    c = new Controller(model);
    r = await c.findByIdAndUpdate(req, res) as any;
  ...
  it('returns 400 from findByIdAndUpdate when nothing is found', async () => {
    req.params.id = goodId;
    req.body = { };
  ...
  it('deleteAllDocs successfully', async () => {
    model.deleteMany = jest.fn(() => Promise.resolve({ message: 'good' }));
    c = new Controller(model);
    const result = await c.deleteAllDocs();
  ...
  it('createDocs successfully', async () => {
    model.create = jest.fn(() => Promise.resolve({ message: 'good' }));
    c = new Controller(model);
    const result = await c.createDocs([{}]);
    expect(result.message).toBe('good');
  });
});
