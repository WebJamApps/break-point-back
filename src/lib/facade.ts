...
interface Ischema {
  ...
  deleteMany:(...args:[Query<Record<string, unknown>, any>])=>({ lean:()=>({ exec:()=>Promise<IdeleteMany | null> }) });
  ...
  find:(...args:[Query<Record<string, unknown>, any>])=>({ lean:()=>({ exec:()=>Promise<Record<string, unknown>[]> }) });
  findOne:(...args:[Query<Record<string, unknown>, any>])=>({ lean:()=>({ exec:()=>Promise<Document | null> }) });
}
...
class Facade {
  ...
  find(query: Query<Record<string, unknown>, any>): Promise<Record<string, unknown>[]> { return this.Schema.find(query).lean().exec(); }

  findOne(query: Query<Record<string, unknown>, any>): Promise<Document | null> { return this.Schema.findOne(query).lean().exec(); }

  deleteMany(query: Query<Record<string, unknown>, any>): Promise<IdeleteMany | null> { return this.Schema.deleteMany(query).lean().exec(); }
